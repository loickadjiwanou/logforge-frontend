...
import { Search, ChevronLeft, ChevronRight, X, RefreshCw, ArrowLeft, FolderKanban, Container, Copy, Hash, Zap } from 'lucide-react';
...
export default function ProjectLogsPage() {
  ...
  const [filters, setFilters] = useState({
    level: '', channel: '', environment: '', search: '', date_from: '', date_to: '', container_name: ''
  });
  ...
  const fetchContainers = useCallback(async () => {
    try {
      const res = await api.get('/logs/docker/containers', { params: { project_id: projectId } });
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      // Dynamically detect if we should show Docker view
      if (!showDockerView && fetchedLogs.some(l => l.metadata?.source === 'docker-agent')) {
  ...
  const clearFilters = () => {
    setFilters({ level: '', channel: '', environment: '', search: '', date_from: '', date_to: '', container_name: '' });
    setPage(1);
  };

  const totalPages = Math.ceil(total / size);
  const hasActiveFilters = filters.search || filters.level || filters.channel || filters.environment || filters.date_from || filters.date_to || filters.container_name;
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {project?.description && (
              <p className="text-sm text-zinc-500 mt-1 ml-7">{project.description}</p>
            )}
            <div className="flex items-center gap-3 mt-1.5 ml-7">
               <div className="flex items-center gap-1.5 bg-zinc-900/50 px-2 py-0.5 rounded border border-zinc-800">
                  <Hash className="w-3 h-3 text-zinc-600" />
                  <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-tight">ID: {project?.id}</span>
                  <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(project?.id); toast.success('Project ID copied'); }}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {showDockerView && (
              <Select value={filters.container_name || 'all'} onValueChange={(v) => updateFilter('container_name', v === 'all' ? '' : v)}>
                <SelectTrigger className="w-[160px] bg-zinc-900 border-zinc-800 text-zinc-300" data-testid="filter-container">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-zinc-800 hover:bg-transparent">
              <TableHead className="text-zinc-500 text-xs font-mono w-[90px]">{t('level')}</TableHead>
              {showDockerView && <TableHead className="text-zinc-500 text-xs font-mono w-[180px]">{t('containers')}</TableHead>}
              <TableHead className="text-zinc-500 text-xs font-mono">{t('message')}</TableHead>
              {!showDockerView && <TableHead className="text-zinc-500 text-xs font-mono w-[110px]">{t('channel')}</TableHead>}
              {showDockerView && <TableHead className="text-zinc-500 text-xs font-mono w-[140px]">Image</TableHead>}
              {!showDockerView && <TableHead className="text-zinc-500 text-xs font-mono w-[100px]">Env</TableHead>}
          ...
          <TableBody>
            {logs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-zinc-600 py-16">
                  {loading ? (
                    <PageLoader />
                  ) : (
              ...
              logs.map(log => (
                ...
                >
                  ...
                  {!showDockerView && <TableCell className="text-xs text-zinc-500">{log.channel}</TableCell>}
                  {showDockerView && (
                    <TableCell className="text-[10px] text-zinc-500 font-mono truncate max-w-[120px]">
                        {log.metadata?.image?.split('/').pop() || '-'}
                    </TableCell>
                  )}
                  {!showDockerView && <TableCell className="text-xs text-zinc-500">{log.environment}</TableCell>}
