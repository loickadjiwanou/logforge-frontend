...
export const AuthProvider = ({ children }) => {
  ...
  const [theme, setThemeState] = useState(
    localStorage.getItem("logforge_theme") || "dark",
  );
    ...
    const initAuth = async () => {
      ...
      if (token) {
        try {
          ...
          setUser(res.data);
          if (res.data.theme && res.data.theme !== themeRef.current) {
  ...
  const login = useCallback(
    async (email, password, company_id = null) => {
  ...
  const signup = useCallback(
    async (email, password, name, invitation_token) => {
